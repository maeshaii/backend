...
def fix_2020_alumni_corrected():
    """Corrected fix for 2020 alumni visibility issue"""
    print("=" * 60)
    print("CORRECTED FIX FOR CLASS OF 2020 ALUMNI")
    print("=" * 60)
    ...
    # Step 2: Fix missing AcademicInfo records with CORRECT field names
    if alumni_without_academic:
        ...
        for alumni in recent_alumni:
            try:
                # Create AcademicInfo with CORRECT field names
                AcademicInfo.objects.create(
                    user=alumni,
                    year_graduated=2020,
                    program='BSIT'  # Use 'program' not 'course'
                )
    ...
    # Step 3: Fix missing TrackerData records with CORRECT field names
    if alumni_without_tracker:
        ...
        for alumni in alumni_without_tracker:
            try:
                # Create TrackerData with CORRECT field names
                TrackerData.objects.create(
                    user=alumni,
                    q_employment_status='pending',
                    tracker_submitted_at=timezone.now()  # Use 'tracker_submitted_at' not 'submitted_at'
                )
