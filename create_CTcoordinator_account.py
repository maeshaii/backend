...
def create_coordinator_account():
    try:
        ...
        username = _get_setting("DEFAULT_CT_COORDINATOR_USERNAME", "CTCOORDINATOR")
        existing_user = User.objects.filter(acc_username=username).first()

        if existing_user:
            print("ℹ️ CT coordinator account already exists!")
            print(f"   Username: {username}")
            password = _get_setting("DEFAULT_CT_COORDINATOR_PASSWORD", "CTWHERENAYOU")
        ...
        coordinator_password = _get_setting(
            "DEFAULT_CT_COORDINATOR_PASSWORD", "CTWHERENAYOU"
        )
        ...
        print("✅ Successfully created CT coordinator account!")
        ...
        print(f"❌ Error creating CT coordinator account: {e}")
