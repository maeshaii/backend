...
def create_coordinator_account():
    try:
        ...
        username = _get_setting("DEFAULT_BSIS_COORDINATOR_USERNAME", "ISCOORDINATOR")
        existing_user = User.objects.filter(acc_username=username).first()

        if existing_user:
            print("ℹ️ BSIS coordinator account already exists!")
            print(f"   Username: {username}")
            password = _get_setting(
                "DEFAULT_BSIS_COORDINATOR_PASSWORD", "ISWHERENAYOU"
            )
        ...
        print("✅ Successfully created BSIS coordinator account!")
        ...
        print(f"❌ Error creating BSIS coordinator account: {e}")
